                    ...
                    <div class="portlet-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <?php
                                    if($row->status==0){
                                        echo '<a href="javascript:;" class="btn btn-xs btn-circle green" id="approve_dtwip_'.$row->id.'" onclick="approve_dtwip('.$row->id.');"> '
                                        . '<i class="fa fa-check"></i> Approve </a> &nbsp; ';
                                        // echo '<a href="javascript:;" class="btn btn-xs btn-circle red" onclick="reject('.$row->id.');"> '
                                        // . '<i class="fa fa-check"></i> Reject </a>';
                                    }else if($row->status==1){
...
function approve_dtwip(id){
    $('#approve_dtwip_'+id).text('Please Wait ...').prop("onclick", null).off("click");
    $.ajax({
        url: "<?php echo base_url('index.php/Tolling/approve_matching_dtwip'); ?>",
        type: "POST",
        data : {dtwip_id: id,so_id: $('#so_id').val()},
        success: function (result){            
            if(result['type_message']=="sukses"){
            //     alert(result['message']);
                location.reload();
