...
<body onload="window.print()">
	...
	<table width="100%" style="page-break-after: auto;">
		<tr>
			<td align="center">
				<h4>Laporan Detail Pemasukan Rongsok per <?=tanggal_indo(date('Y-m-d', strtotime($_GET['ts']))).' sampai '.tanggal_indo(date('Y-m-d', strtotime($_GET['te'])));?></h4>
			</td>
		</tr>
	</table>
	<table width="100%" cellpadding="2" cellspacing="0" style="font-size: 13px;">
		<thead>
			<tr>
                ...
                <th style="border-top: 1px solid; border-bottom: 1px solid;">No Palette</th>
				...
				if ($last_series != '') {
					if ($last_series != $row->no_ttr) {
						...
						if ($last_sumber == $row->sumber) {
							echo 
								"<tr>
									<td colspan='9'><b><u>".$row->no_ttr."</u></b></td>
								</tr>";
						} else {
							$last_series = '';
						}							
