<html lang="en">
    ...
    <body class="margin-left:40px;">
        <p>&nbsp;</p>
        <h3 style="text-align: center; text-decoration: underline;"><?php if($this->session->userdata('user_ppn')==1){ echo 'PT. KAWAT MAS PRAKASA<br>'; }?>PACKING LIST SEMENTARA<br>RONGSOK</h3>
            ...
            <tr><td colspan="3">
                    <table border="0" cellpadding="4" cellspacing="0" width="100%">
                        <tr>
                            ...
                            <td colspan="3" style="text-align:center; border-left:1px solid #000; border-bottom:1px solid #000; border-top:1px solid #000; border-right:1px solid #000;"><strong>Qty</strong></td>
                        </tr>
                        <tr>
                            ...
                            foreach ($details as $row){
                                if($row->nama_item!=$last_series && $last_series!=null){
                                    ...
                                    echo '<td style="text-align:right; border-left:1px solid #000; border-top:1px solid #000; border-bottom:1px solid #000">
                                        ...
                                        <td style="text-align:right; border-left:1px solid #000; border-right:1px solid #000; border-bottom:1px solid #000; border-top:1px solid #000; border-right:1px solid #000;"></td></tr>';
                                    $bruto = 0;
                                    $bobin = 0;
                                    $netto = 0;
                                    $no = 1;
                                ...
                                echo '<td style="border-left:1px solid #000">'.$row->kode_rongsok.'</td>';
                                ...
                                echo '<td style="text-align:right; border-left:1px solid #000">'.number_format($row->berat_palette, 2, '.', ',').'</td>';
                                echo '<td style="text-align:right; border-left:1px solid #000; border-right:1px solid #000;">'.number_format($row->netto, 2, '.', ',').' '.$row->uom.'</td>';
                                echo '</tr>';
                                ...
                                $bruto += $row->bruto;
                                $bobin += $row->berat_palette;
                                $netto += $row->netto;
                                $total_bruto += $row->bruto;
                                $total_bobin += $row->berat_palette;
                        ...
                        <tr>
                            <td style="text-align:right; border-left:1px solid #000; border-bottom:1px solid #000;" colspan="4"><strong>Total</strong></td>
                        ...
                        <tr>
                            <td style="text-align:right; border-left:1px solid #000; border-bottom:1px solid #000;" colspan="4"><strong>Grand Total</strong></td>
                            <td style="text-align:right; border-left:1px solid #000; border-bottom:1px solid #000">
