<div class="row">
    <div class="col-md-12 alert-warning alert-dismissable">        
        <h5 style="color:navy">
            ...
            <a href="<?php echo base_url('index.php/StokOpname/'); ?>"> Adjusment</a>
        </h5>          
    </div>
</div>
...
<div class="row">                            
    <div class="col-md-12">
        ...
        <form class="eventInsForm" method="post" target="_self" name="formku" 
              id="formku" action="<?php echo base_url('index.php/StokOpname/saveAdjustment'); ?>">  
            <div class="row">
                <div class="col-md-5">
                    <div class="row">
                        <div class="col-md-4">
                    ...
                    <div class="table-scrollable">
                        <table class="table table-bordered table-striped table-hover" id="tabel_dtr">
                            ...
                            <tr>
                                ...
                                <td><input type="text" id="berat_bobbin_1" name="myDetails[1][berat_bobbin]" class="form-control myline" value="0"  maxlength="10"></td>
                                ...
                                <td><input type="text" name="myDetails[1][no_bobbin]" id="no_bobbin_1"class="form-control myline" onkeyup="this.value = this.value.toUpperCase()" value="0"></td>
...
function timbang_netto(id){
    var no_packing = $('#no_packing_'+id).val();
    size = no_packing.substring(7,6);
    if (size == "K") {
        size = size+"A";
        nomor = no_packing.substring(15,12);
    } else {
        nomor = no_packing.substring(15,11);  
    }
    no_bobbin = size+nomor;
    
    // console.log(no_bobbin);

    $.ajax({
        url: "<?php echo base_url('index.php/StokOpname/getBobbin'); ?>",
        type: "POST",
        data: {no_bobbin: no_bobbin},
        dataType: "json",
        success: function(result) {
            console.log(result);
            var bruto = $("#bruto_"+id).val();
            var berat_palette = result['bobbin']['berat'];
            var total_netto = bruto - berat_palette;
            var total = total_netto.toFixed(2);
            $('#berat_bobbin_'+id).val(berat_palette);
            $("#netto_"+id).val(total);
            $("#no_bobbin_"+id).val(result['bobbin']['nomor_bobbin']);
        }
    });
...
function saveDetail(id){
    ...
    //     $('.alert-danger').show(); 
        ...
        $("#no_packing_"+id).attr('readonly','readonly');
        ...
        $("#tabel_dtr>tbody").append(
            '<tr>'+
                ...
                '<td><select id="name_rongsok_'+new_id+'" name="myDetails['+new_id+'][nama_item]" class="form-control select2me myline" data-placeholder="Pilih..." style="margin-bottom:5px" onchange="get_uom_po(this.value,'+new_id+');">'+
                    '<option value=""></option>'+
                    '<?php foreach($list_fg as $v){ print('<option value="'.$v->id.'">('.$v->kode.') '.$v->jenis_barang.'</option>');}?>'+
                '</select>'+
                '</td>'+
                ...
                '<td><input type="text" id="netto_'+new_id+'" name="myDetails['+new_id+'][netto]" class="form-control myline" value="0" maxlength="10"></td>'+
                ...
                '<td><input type="text" name="myDetails['+new_id+'][no_bobbin]" id="no_bobbin_'+new_id+'"class="form-control myline" onkeyup="this.value = this.value.toUpperCase()" value="0"></td>'+
