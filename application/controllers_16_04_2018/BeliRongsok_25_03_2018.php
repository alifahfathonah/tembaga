...
class BeliRongsok extends CI_Controller{
    ...
    function save(){
        ...
        $code = $this->Model_m_numberings->getNumbering('PORS', $tgl_input); 
        
        if($code){        
            $data = array(
                'no_po'=> $code,
                'tanggal'=> $tgl_input,
                'ppn'=> (!empty($this->input->post('flag_ppn'))? 1: 0),
            ...
            $this->session->set_flashdata('flash_msg', 'PO rongsok gagal disimpan, penomoran belum disetup!');
            redirect('index.php/BeliRongsok');
        }
    }    
