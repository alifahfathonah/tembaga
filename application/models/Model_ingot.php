...
class Model_ingot extends CI_Model{
    ...
    function show_hasil($id){
        $data = $this->db->query("Select thm.*, pi.no_produksi, pi.remarks, a.tipe_apolo, jb.jenis_barang, jb.uom, usr.realname as pic, tbw.id as id_bpb_wip, tba.id as id_bpb_ampas, tba.status as status_ampas, dtr.status as status_dtr, dtr.id as id_dtr, dtr.status as status_dtr, thw.id as id_hasil_wip
    ...
    function show_detail_spb_fulfilment_approved($id){
        $data = $this->db->query("Select dtrd.id as id_detail, rsk.nama_item, rsk.uom, rsk.kode_rongsok, spdf.id, dtrd.no_pallete, dtrd.netto, COALESCE(NULLIF(dtrd.so_id,0),dtrd.retur_id) as so_id, dtrd.flag_sj, sr.stok_netto as stok, dtrd.line_remarks, dtrd.tanggal_keluar
